...
let niceInvoice = async (invoice, path) => {
...
let header = (doc, invoice) => {
  ...
  if (fs.existsSync(invoice.header.company_logo)) {
    doc.image(invoice.header.company_logo, 50, 45, { width: 150 })
      .fontSize(20)
...
let invoiceTable = (doc, invoice) => {
  ...
  for (i = 0; i < invoice.items.length; i++) {
    ...
    tableRow(
      ...
      item.purpose,
      "", //item.description,
      item.mode, //formatCurrency(item.price, currencySymbol),
      "",// item.quantity,
      // formatCurrency(applyTaxIfAvailable(item.price, item.quantity, item.tax), currencySymbol),
      formatCurrency(applyTaxIfAvailable(item.fees, 1, item.tax), currencySymbol),
